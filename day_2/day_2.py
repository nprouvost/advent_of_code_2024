...
def main():
        ...
        if not check1[i]:
            ...
            if test_general_decreasing > test_general_increasing:
                ...
                if report[test_decreasing[0][0]] < sorted_reports_decreasing[i][test_decreasing[0][0]]:
                    ...
                    print("possibility 2 " + "decreasing failed", i, "filtered index", false_index, "other case would be", test_decreasing[0][0])
                    ...
                    if ak.sum(report == sorted_reports_decreasing[i][test_decreasing[0][0]]) > 1:
                        ...
                        false_index = ak.argmax(report_without_one_double == sorted_reports_decreasing[i][test_decreasing[0][0]]) + 1
                else:
                ...
                if len(test_increasing) > 0:
                    if report[test_increasing[0][0]] > sorted_reports[i][test_increasing[0][0]]:
                        ...
                        if ak.sum(report == sorted_reports[i][test_increasing[0][0]]) > 1:
                            report_without_one_double = ak.concatenate([report[:false_index], report[false_index + 1:]])
                            false_index = ak.argmax(report_without_one_double == sorted_reports[i][test_increasing[0][0]]) + 1
                    else:
    ...
    for i, report in enumerate(filtered_reports):
        print("valid_filtered_reports", (new_check1 & ~new_check2)[i], i, "check 1", new_check1[i], "check 2", ~new_check2[i])
    ...
    #         if ak.all(sorted_filtered_report_decreasing == filtered_report) or ak.all(sorted_filtered_report == filtered_report):
    #             differences = ak.Array([abs(filtered_report[j] - filtered_report[j + 1]) for j in range(len(filtered_report) - 1)])
